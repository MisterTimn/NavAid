...
void CannyThreshold(int, void*, bool isHorz) {
    ...
    for (int i = 0; i < lines.size(); i++) {
        ...
        if (!isHorz && (std::abs(theta) < 0.3 || std::abs(theta - 3.14) < 0.5 || std::abs(theta - 6.28) < 0.5)) {
            vert.push_back(cvRound(x0));
        }
        // theta == dicht bij pi/2 of 3pi/2 -> horizontaal
        else if (isHorz && (std::abs(theta - 1.57) < 0.5 || std::abs(theta - 4.71) < 0.5)) {
...
int main(int argc, char** argv) {
    ...
    warpPerspective(src, src, transformationMatrix, src.size());
    ...
    //region-of-interest
    roi = cv::Mat(saturation, cv::Rect(src.cols / 2.7, src.rows / 2, src.cols / 2.5, src.rows / 3));

    //blur
    cv::GaussianBlur(roi, roi, cv::Size(), 1.0, 1.0);
